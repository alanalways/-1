...
async function loadTradingViewWidget(symbol) {
    ...
    try {
        // Fetch historical data from static JSON (generated by GitHub Actions)
        // Ensure symbol matches filename (e.g. 6770.TW -> 6770)
        const cleanCode = symbol.replace('.TW', '').replace('.TWO', '');
        // Try exact match first, then clean
        let url = `data/history/${symbol}.json`;

        // Check if file exists (HEAD request or just fetch) -> actually standard fetch is fine
        // Note: My fetch script saves as "2330.json" (clean code? let me re-verify)
        // Re-verification: fetch-historical.js saves as `${symbol}.json`. 
        // And symbol passed in was cleanCode. So it is "2330.json".

        url = `data/history/${cleanCode}.json`; // Assume clean code naming
        ...
        console.warn('static chart fetch failed:', error);
        if (loading) loading.innerHTML = `<span style="color: var(--accent-yellow);">📊 資料更新中 (${error.message})...</span>`;
        // Fallback to "Snapshot" point if available? Or just show error.
    }
}
